...
export default function ListComponent({
  onClickProductToEdit,
  isEditOrDeleteButtonClicked,
}: {
  onClickProductToEdit: (item: IProductList) => void;
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3">
          {(search ? newProductList : productList).map((item) => (
            <div
              key={item.name}
              className={`bg-slate-500 m-2 p-2 flex items-center gap-2 w-fit ${isEditOrDeleteButtonClicked && "cursor-pointer hover:opacity-50"}`}
              onClick={isEditOrDeleteButtonClicked ? () => onClickProductToEdit(item) : undefined}
            >
